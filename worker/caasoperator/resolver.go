...
// ResolverConfig defines configuration for the caasoperator resolver.
...
type caasoperatorResolver struct {
	config                ResolverConfig
	retryHookTimerStarted bool
}

// NewCaasOperatorResolver returns a new resolver.Resolver for the caasoperator.
func NewCaasOperatorResolver(cfg ResolverConfig) resolver.Resolver {
	return &caasoperatorResolver{
		config:                cfg,
		retryHookTimerStarted: false,
	}
}

func (s *caasoperatorResolver) NextOp(
...
func (s *caasoperatorResolver) nextOpConflicted(
	localState resolver.LocalState,
	remoteState remotestate.Snapshot,
	opFactory operation.Factory,
) (operation.Operation, error) {
...
func (s *caasoperatorResolver) nextOpHookError(
	localState resolver.LocalState,
	remoteState remotestate.Snapshot,
	opFactory operation.Factory,
) (operation.Operation, error) {
...
func (s *caasoperatorResolver) nextOp(
	...
	case params.Dying:
		...
		fallthrough
	case params.Dead:
		// The unit is dying/dead and stopped, so tell the caasoperator
		// to terminate.
		return nil, resolver.ErrTerminate
	}
