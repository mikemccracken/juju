...
func newStateForVersionFn(version int) func(base.APICaller, names.ApplicationTag) *State {
	return func(caller base.APICaller, authTag names.ApplicationTag) *State {
...
func (st *State) CAASApplication(tag names.ApplicationTag) (*CAASApplication, error) {
	life, err := st.life(tag)
	if err != nil {
		return nil, err
	}
